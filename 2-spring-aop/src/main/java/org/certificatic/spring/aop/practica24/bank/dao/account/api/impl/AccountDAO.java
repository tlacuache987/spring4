...
public class AccountDAO implements IAccountDAO {
	...
	public List<Account> findByCustomerId(Long customerId) {
		log.info("Inside accountDAO.findByCustomerId(). Finding accounts for customer: {}", customerId);

		log.info("Finding Accounts ...");

		List<Account> result = new ArrayList<>();
		result.add(Account.builder().accountNumber("000001").accountDescription("Account 1").build());
		return result;
	}

	@Override
	public void updateBalance(Account account, Long amount) {
		log.info("Inside accountDAO.updateBalance(). Account: {}, ammount: {}", account.getAccountNumber(),
				amount);
	...
	public void updateDescription(Account account) {
		log.info("Inside accountDAO.updateDescription(). Updating account [{}] description to: {}",
